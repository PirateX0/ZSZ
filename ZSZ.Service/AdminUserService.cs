...
namespace ZSZ.Service
{
    ...
    {
        public bool isPhoneNumeExistent(string phoneNum)
        {
            ...
            using (ZSZDbContext ctx = new ZSZDbContext())
            {
                ...
                //新增时，手机号唯一，不能重复
                isExistent = bs.GetAll().Any(u => u.PhoneNum == phoneNum);


                return isExistent;
            }
        }
        public bool isPhoneNumeExistent(string phoneNum, long userId)
        {
            bool isExistent = false;
            using (ZSZDbContext ctx = new ZSZDbContext())
            {
                ...
                //编辑，手机号唯一，不能和自身之外的重复
                isExistent = bs.GetAll().Any(u => u.PhoneNum == phoneNum && u.Id != userId);
            ...
            string password, string email, long? cityId)
        {
            ...
            user.Email = email;
            user.Name = name;
            if (isPhoneNumeExistent(phoneNum))
            {
